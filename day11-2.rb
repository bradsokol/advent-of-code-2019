...
code = File.open('day11.txt').readline.split(',').map(&:to_i)
...
y = 0
direction = 0
robot = Computer.new(code)
state = 0

robot.compute(1) do |output|
  if state == 0
    grid[[x, y]] = output
    state = 1
  else
...
min_y = grid.keys.map { |panel| panel[1] }.min
max_y = grid.keys.map { |panel| panel[1] }.max
